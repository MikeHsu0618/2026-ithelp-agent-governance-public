...
- [Day 20–24｜Agent Traceability Pipeline](labs/04-telemetry-pipeline/README.md)：先用一筆 deterministic no-op action 拆開 Application Record、Operational Telemetry 與 Governance Event，再接通 Gateway、Runtime、MCP 與 LGTM。後續實測身分欄位分流、Cardinality，以及 LLM Fallback 的原生成本證據和帳單邊界。
