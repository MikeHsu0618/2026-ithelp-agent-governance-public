...
| RR-02 | Human、Service 與實際 Workload instance 可能只共用 client identity，無法做 instance attribution | Day 7／27：Workload identity 為 `UNKNOWN`；現有控制為 AWS Cognito Human／M2M、Token validation 與 Runtime context | IT／Identity + Platform／SRE | 受影響服務的 Business Owner；需要 per-instance policy 時不得繼續沿用 shared client attribution | 出現長效 client secret、跨 namespace 共用 credential 或 per-instance policy 時，以 workload-bound Token 或同等機制完成 live E2E |
