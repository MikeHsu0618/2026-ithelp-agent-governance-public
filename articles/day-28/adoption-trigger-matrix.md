...
|---|---|---|---|---|---|---|
| 單一團隊、只讀 Agent | Human／M2M identity、bounded credential、Tool outcome、action／trace correlation | AWS Cognito Human／M2M、Git-owned BYO Runtime、Alloy／LGTM | Identity／Platform + Application + SRE | 集中式 Gateway、kagent、Registry、額外的稽核儲存平台 | Git-owned BYO Runtime + LGTM | Tool 出現副作用、共享 traffic 或明確的法遵要求時重開；參考 Lab 01／02／04 |
...
| Human／M2M access 已足夠，企業 lifecycle owner 未成立 | 維護 callback、scope、claim、client secret、Gateway policy 與 offboarding | AWS Cognito User Pools | Identity／IT + Platform | AI 專用 Keycloak Identity Center | Managed identity bridge | 企業 joiner／mover／leaver、SaaS／下游整合範圍與 owner 同時成立時重評；使用實務 E2E + Lab 02 |
| 事故調查要定位 Pod，或下游政策要驗工作負載、移除長效 secret | 前者先關聯 Pod UID；後者再評估可驗證的 bound credential、issuer／audience 與 rotation | 尚未選定；Kubernetes bound Token／SPIFFE 類機制待評估，ServiceAccount 名稱不足以辨識唯一 Pod | 尚未指定；Platform + Security + Application | 在沒有需求與 live evidence 時先不宣稱支援 | `UNKNOWN` | Per-instance auth 或 secretless policy 成為需求後跑 live E2E |
