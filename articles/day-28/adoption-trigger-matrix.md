...
|---|---|---|---|---|---|---|
| 單一團隊、只讀 Agent | Human／M2M identity、bounded credential、Tool outcome、action／trace correlation | Cognito Human／M2M、Git-owned BYO Runtime、Alloy／LGTM | Identity／Platform + Application + SRE | 集中式 Gateway、kagent、Registry、tamper-evident Audit | Git-owned BYO Runtime + LGTM | Tool 出現副作用、共享 traffic 或 evidence obligation 時重開；以 Lab 01／02／04 contract 驗收 |
...
| 事件需要不可否認、legal hold 或長期完整性保存 | Event-time signature、hash chain、append-only／WORM、access audit、retention | 尚無已驗證實作 | Security／Risk + Platform | 不把 operational LGTM 冒充證據庫 | LGTM 保持 operational telemetry；Audit 為 `UNKNOWN` | 先指定保存目的、威脅模型與 owner，再驗證 ingestion、storage、query 與 privileged deletion |
