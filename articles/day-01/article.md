...
![不可信 Log 進入 ADK Agent context，Gemini 提出危險 Tool Call；open policy 放行後，Function Tool 追加安全 canary event。](https://raw.githubusercontent.com/MikeHsu0618/2026-ithelp-agent-governance-public/day-28/assets/diagrams/day-01/unsafe-action-chain.png)
...
![Gemini live run 選擇 delete_demo_database，open policy 放行後 safe canary 增加一筆。](https://raw.githubusercontent.com/MikeHsu0618/2026-ithelp-agent-governance-public/day-28/assets/screenshots/day-01/01-live-unsafe-tool-call.png)
...
這筆 `trace_id` 會留到 Day 26。屆時會從本機 Artifact 誠實回放當時真正留下的欄位，再另跑一筆現行 Alloy／LGTM 事件驗證今天的 telemetry path。兩組資料可以比較，不能假裝是同一場事故。

## 這三十天會走到哪裡

這個系列不會每天換一套熱門工具。我會沿著同一條 action path，逐步補上缺少的控制與證據。

![三十天沿著同一條 action path 分成五個階段，從 Agent 攻擊面走到 Identity、執行控制、Traceability 與組織落地。](https://raw.githubusercontent.com/MikeHsu0618/2026-ithelp-agent-governance-public/day-28/assets/diagrams/day-01/series-route.png)
...
完整 source code、測試與操作說明都放在 [直接進入 Day 1 Lab](https://github.com/MikeHsu0618/2026-ithelp-agent-governance-public/blob/day-28/labs/01-unsafe-agent/README.md)。不想先 clone 整個 repo，也可以從這裡確認目錄與執行條件。
