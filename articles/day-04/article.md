...
目前的 policy 只收到 `tool.name`。同一個 callback 明明拿得到 Tool arguments，卻只把它們寫進事件，沒有交給授權判斷：
...
> `Lab` Day 4 沿用前一天的 [Google ADK Lab](https://github.com/MikeHsu0618/2026-ithelp-agent-governance-public/blob/day-04/labs/01-unsafe-agent/README.md) 與 attack scenario，不另造一套 runtime。檢查焦點從模型是否犯錯，移到現有證據走到 policy 前還剩多少。
...
雖然這三個欄位都有值，來源卻不足以把它們當成已驗證身分。`synthetic-user-sre-oncaller` 只是 session label，沒有 issuer、authentication method 或 credential 能證明它對應到哪位使用者。`sre_investigation_agent` 是載入的 Agent 名稱，不是實際送出 request 的 workload identity。`payments-demo` 則是模型提出的 argument，尚未經 policy 驗證。
...
「資料不存在」和「資料沒有送進 policy」需要不同修法。前者要由 IdP、runtime 或 workload identity 補上，後者要修改 policy contract。
...
![值班工程師要求 SRE Agent 調查 latency。Agent 讀到惡意 Log 後提出 delete_demo_database，Runtime workload 在呼叫 requested target 前經過 Policy checkpoint。現有 policy 只收到 Tool name。](https://raw.githubusercontent.com/MikeHsu0618/2026-ithelp-agent-governance-public/day-04/assets/diagrams/day-04/confused-deputy-sequence.png)
...
因此 identity、delegation 與 execution 不能共用一個模糊的 `actor` 欄位：
...
我把現有 Lab 填進 [Agent Delegation Decision Table](https://github.com/MikeHsu0618/2026-ithelp-agent-governance-public/blob/day-04/articles/day-04/delegation-decision-table.md)。這次不只寫值，還要標明證據狀態，以及 policy 現在是否看得到：
...
Decision Table 裡的 `UNKNOWN` 只留給真的沒有證據的欄位。Session label、Agent metadata 與 Tool argument 都有值，只是可信度和用途不同。把這三種狀態混成「我們都有記 Log」，design review 就很容易錯過 policy 實際上只收到 `tool_name`。
...
對這篇來說，RFC 8693 的價值是證明 subject 與 current actor 本來就可以分開表達。是否真的導入 Token Exchange，要等 IdP 能力、audience boundary 與 credential flow 一起評估。Day 4 不先替平台選答案。
