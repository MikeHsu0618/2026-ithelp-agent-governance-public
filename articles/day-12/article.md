...
<img src="https://raw.githubusercontent.com/MikeHsu0618/2026-ithelp-agent-governance-public/day-23/assets/third-party/aws/amazon-cognito-architecture-icon.png" alt="Amazon Cognito 官方 AWS Architecture Icon" width="96">
...
完整設定在 [agentgateway-cognito.yaml](https://github.com/MikeHsu0618/2026-ithelp-agent-governance-public/blob/day-23/labs/02-identity-boundary/configs/agentgateway-cognito.yaml)。Pinned image 實際載入 committed JWKS、YAML 與 CEL 後得到：
...
完整範例還包含 user pool、managed-login domain、resource server、custom scope、token lifetime 與 outputs，放在 [cognito-terraform](https://github.com/MikeHsu0618/2026-ithelp-agent-governance-public/tree/day-23/labs/02-identity-boundary/configs/cognito-terraform/)。本次執行範圍如下：
...
![Day 12 Cognito 雙路徑 Lab 的實際 CLI 結果。Human 與 M2M 各有一條成功 path，callback、scope、policy claim、public client、client secret 與 M2M resource binding 錯誤都被分階段拒絕。](https://raw.githubusercontent.com/MikeHsu0618/2026-ithelp-agent-governance-public/day-23/assets/screenshots/day-12/01-cognito-dual-path-results.png)
...
九組結果是 9/9 matched，完整 Lab 共有 75 tests，branch coverage 90.81%，Ruff lint／format clean。Terraform 與 agentgateway config validation 也另外通過。Compact JWT、PKCE verifier、client secret 與 private key 都不落盤，Artifact 只保存合成 claims、safe registration、SHA-256 fingerprint、decision 與 manifest。故障時可以配合 [Human／M2M 雙路徑盤點表](https://github.com/MikeHsu0618/2026-ithelp-agent-governance-public/blob/day-23/articles/day-12/cognito-dual-path-checklist.md)逐層檢查，完整的驗證紀錄則收在 [Day 12 evidence](https://github.com/MikeHsu0618/2026-ithelp-agent-governance-public/blob/day-23/assets/screenshots/day-12/evidence.md)。
