...
把它叫做 Prompt Injection 沒有錯。不過如果問題只停在這個名稱，修補的位置很容易只剩 Prompt、關鍵字與輸入過濾。

這三筆 event 顯示的是另一件事：不可信 Log 先影響模型，模型再提出動作，`open` policy 接著放行，最後才由 Tool 觸發 safe canary。Prompt Injection 只是入口；每往後一步，系統其實都還有一次拒絕的機會。

所以 Day 2 不換 Agent，也不再造一場攻擊。我沿用 Day 1 的 `trace_id=a281375fdcb5516c8983eada8ff11c9b`，把同一次執行拆成 trust boundary，找出這條路徑上究竟有哪些決策沒有被做。

本文仍是合成 Lab，不是真實事故。Threat Model 的邊界來自這筆 [公開的 ordered events](https://github.com/MikeHsu0618/2026-ithelp-agent-governance-public/blob/day-02/assets/screenshots/day-01/evidence/live-events.jsonl)；OWASP 與 NIST 的官方資料只用來校正威脅名稱。

## 一條 Trace，七道信任邊界

我先不畫企業架構，也不急著放 Gateway、IdP 或 MCP。只沿著 event 發生的順序，逐段追問「上一層的信任，能不能直接帶到下一層」。
...
七只是方便後文引用；真正需要保留的是 `UNKNOWN`。Day 1 只證明 TB-02、TB-04、TB-05 與 TB-07 的行為；沒有 credential 的 canary，不能拿來推論正式環境的資料庫權限。缺證據就先寫不知道，後面的 Identity、Policy 與 Audit 才知道要補什麼。
...
![一般 Web request 依程式碼固定路徑執行；Agent run 則加入外部資料、模型 action proposal、獨立授權與 Tool result 回饋迴圈。](https://raw.githubusercontent.com/MikeHsu0618/2026-ithelp-agent-governance-public/day-02/assets/diagrams/day-02/web-vs-agent-attack-surface.png)

新增的難題落在 data path 和 action path 交疊。外部內容既能影響下一個動作，動作結果又會成為後續判斷的輸入。
...
失守之後會造成多大損害，則更接近 [OWASP LLM06:2025 Excessive Agency](https://genai.owasp.org/llmrisk/llm062025-excessive-agency/) 拆出的三個根因：
...
這個拆法會改變修補順序。Input guard 可以過濾已知 pattern、標記不可信來源，也值得做；但它回答的是「這段內容像不像攻擊」。它不能替某個 actor 對某個 resource 做 authorization。

即使模型照樣被騙，最小 Tool catalog、最小 credential 權限、獨立 policy 與必要的人工核准，仍能分別縮小 blast radius。安全性不該全部押在模型永遠判斷正確。

## 把 Threat Model 寫成可審查的 Worksheet

架構圖適合找邊界，真正進 design review 時，還是需要一份能填寫、能留下 `UNKNOWN` 的盤點表。我把 Day 1 的拆解整理成 [Agent Threat Model Worksheet](https://github.com/MikeHsu0618/2026-ithelp-agent-governance-public/blob/day-02/articles/day-02/threat-model-worksheet.md)，內容分成六組：
...
寫完整後，漏洞名稱不再是結論，而是定位問題的索引。

## 下一步：讓 Policy 真正拒絕 Tool

Day 2 已經把 Day 1 的危險路徑拆開：惡意內容從 TB-02 進入，模型在 TB-04 提出動作；真正讓副作用發生的，卻是 TB-05 沒有獨立授權，以及 TB-06 尚未被證明的 resource-side 權限。

下一篇繼續使用同一個 Agent 和 safe canary，只加入兩種控制：keyword guard 與 Tool allowlist。明顯攻擊會被 guard 擋住；改寫後的內容若通過輸入檢查，獨立的 Tool authorization 仍必須在執行前說 `DENY`。
