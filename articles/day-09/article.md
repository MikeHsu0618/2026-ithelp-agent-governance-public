...
![值班工程師請 Copilot 調查，Copilot 委派 Investigator，再由 Kubernetes workload 呼叫 query_logs。圖下方是同一次 Tool Call 保存的 actor chain、credential、target 與事件關聯欄位。](https://raw.githubusercontent.com/MikeHsu0618/2026-ithelp-agent-governance-public/day-28-r1/assets/diagrams/day-09/delegation-sequence.png)
...
公開 Lab 的 [Human delegated 範例](https://github.com/MikeHsu0618/2026-ithelp-agent-governance-public/blob/day-28-r1/assets/screenshots/day-09/evidence/demo-context-human-delegated.json) 將這次 `query_logs` 留成一筆事件。`actor_chain` 記下值班工程師、Copilot、Investigator 與執行的 Workload。`credential` 記當前觀測點所見的 issuer、subject、client、audience 與指紋，`target` 則記這次碰的是哪個 resource、哪個 action。`event_id`、時間與 `trace_id` 方便回頭找同一次動作。完整欄位與 schema 放在 [Delegation Context Field Guide](https://github.com/MikeHsu0618/2026-ithelp-agent-governance-public/blob/day-28-r1/articles/day-09/delegation-context-field-guide.md)，正文先拿它回答值班時最直接的問題。
...
[Lab 02](https://github.com/MikeHsu0618/2026-ithelp-agent-governance-public/blob/day-28-r1/labs/02-identity-boundary/README.md) 使用合成的值班工程師、Agent 與 ServiceAccount，檢查這份資料契約是否能區分上述情境。在 repo root 執行：
...
下面是最能看出差異的四組結果。其餘負向案例與原始紀錄在 [Day 9 evidence](https://github.com/MikeHsu0618/2026-ithelp-agent-governance-public/blob/day-28-r1/assets/screenshots/day-09/evidence.md)。
...
![Delegation Context Lab 的七組實際 CLI 結果。Human delegated、排程與明確標示未知身分的案例通過。缺欄位、null、Agent 順序錯誤及 actor-only 紀錄被拒絕。](https://raw.githubusercontent.com/MikeHsu0618/2026-ithelp-agent-governance-public/day-28-r1/assets/screenshots/day-09/01-delegation-context-results.png)
