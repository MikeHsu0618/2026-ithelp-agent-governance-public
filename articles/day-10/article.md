...
![值班工程師的入口 Token 到了 Observability MCP。嚴格檢查 audience 時拒絕。改讓下游接受入口 audience 時雖能通過，Audit 卻只剩 Human。另一條路改用下游專用 Token 和 Delegation Context。](https://raw.githubusercontent.com/MikeHsu0618/2026-ithelp-agent-governance-public/day-10-r3/assets/diagrams/day-10/passthrough-vs-bound-token.png)
...
下游因此能驗證眼前這枚 Token 以 runtime client 為 subject，而且是發給自己的。Day 9 的 Delegation Context 另外保存「這筆查詢最初由值班工程師交辦，經過哪版 Agent」。兩者放在同一筆 Audit 裡，可以同時看到：
...
| 最後一跳要操作什麼 | `mcp://lab/observability/query` 的 `query_logs` |

Runtime 的 OAuth client 是這一跳 Token 的 subject，不等於 Day 9 紀錄裡的交辦者。Lab fixture 也另有 Kubernetes ServiceAccount metadata，但那只表示它宣稱使用哪個工作負載身分，不能推論 MCP 已驗過這枚 Kubernetes 憑證，更不能用來辨識唯一的 Pod。Lab 的 `FULL_CHAIN` 表示 Context 與下游 Token 欄位能對上，並不替上游每個角色完成身分驗證。
...
完整的離線對照與重跑指令留在 [Lab 02](https://github.com/MikeHsu0618/2026-ithelp-agent-governance-public/blob/day-10-r3/labs/02-identity-boundary/README.md) 和 [Day 10 evidence](https://github.com/MikeHsu0618/2026-ithelp-agent-governance-public/blob/day-10-r3/assets/screenshots/day-10/evidence.md)。[Token Passthrough Audit Reading Guide](https://github.com/MikeHsu0618/2026-ithelp-agent-governance-public/blob/day-10-r3/articles/day-10/token-passthrough-audit-guide.md) 可用來比較兩條 `ALLOW` 路徑的 Audit。下圖呈現七組合成請求的結果。讀者可先看嚴格 audience 拒絕、共用 audience 放行但歸屬消失、下游專用 Token 與 Context 保留責任鏈這三組。

![Day 10 的七組實際 CLI 結果。入口 Token 在自己的 resource 通過，下游嚴格驗 audience 時拒絕。共用 audience 雖通過但只剩 Human 歸屬，下游專用 Token 加 Context 才保留完整責任鏈。](https://raw.githubusercontent.com/MikeHsu0618/2026-ithelp-agent-governance-public/day-10-r3/assets/screenshots/day-10/01-passthrough-results.png)

這裡用本機 issuer 與兩個離線 protected-resource validator 測授權結果，沒有啟動兩個 HTTP MCP Server，也沒有實作 AWS Cognito 或 Token Exchange。下游 Token 由本機 issuer 直接簽出，只用來比較 Token 與 Context 應具有的性質。
