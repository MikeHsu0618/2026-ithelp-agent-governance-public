...
![grafana-claudestats-app 的 Overview 畫面。成本、Token 與 Session 適合掌握使用情況，還不能單獨回答一次工具操作的責任。](https://raw.githubusercontent.com/MikeHsu0618/2026-ithelp-agent-governance-public/day-20-r4/assets/screenshots/day-20/claude-code-stats-overview.png)
...
![grafana-claudestats-app 的 Activity 畫面。程式碼變更、Commit 與 PR 提供 Code Agent 的操作脈絡，畫面使用可公開的測試資料。](https://raw.githubusercontent.com/MikeHsu0618/2026-ithelp-agent-governance-public/day-20-r4/assets/screenshots/day-20/claude-code-stats-activity.png)
...
[Lab 04 README](https://github.com/MikeHsu0618/2026-ithelp-agent-governance-public/blob/day-20-r4/labs/04-telemetry-pipeline/README.md) 保留重跑方式。這裡固定執行一筆 Deterministic No-op Canary：Tool 名稱仍是 `delete_demo_database`，程式不會連資料庫、Shell、Kubernetes 或外部 API，只寫出 `CANARY_TRIGGERED` Receipt，`side_effects` 固定為 `0`。讀者不需要 Gemini Key，也能比較同一筆 Action 留下的欄位。
...
![Grafana Tempo 實際查到的 Day 20 trace。根 span 是 invoke_agent sre-investigation-agent，child span 是 execute_tool delete_demo_database，兩者共用 trace ID。](https://raw.githubusercontent.com/MikeHsu0618/2026-ithelp-agent-governance-public/day-20-r4/assets/screenshots/day-20/tempo-agent-tool-trace.png)
...
Human Principal 由 Fixture IdP 提供。Workload 則只有 Runtime Fixture 自行填入。兩個值出現在同一份 Event，來源卻不同，所以欄位分別寫 `VERIFIED` 和 `ASSERTED`。若只是要定位哪個 Pod 產生事件，可以把 [Pod UID](https://opentelemetry.io/docs/specs/semconv/resource/k8s/) 接進 telemetry。若要讓下游相信某個 Pod 的身分，才需要可由下游驗證的工作負載憑證。這份 Fixture 兩件事都沒有實作。
...
完整欄位與放置位置收在 [Governed Action Field Set v0.1 放置指南](https://github.com/MikeHsu0618/2026-ithelp-agent-governance-public/blob/day-20-r4/articles/day-20/governed-action-field-guide.md)。想重跑的人可以直接改 Fixture，看看拿掉 Policy Version 之後事件在哪裡被擋下。
