...
![同一批 90 筆 request 通過三組平行 agentgateway。Metrics 依序只加入 team、加入 user_id、再加入 conversation_id，Tempo 與 Loki 保留單筆身分查詢。](https://raw.githubusercontent.com/MikeHsu0618/2026-ithelp-agent-governance-public/day-28/assets/diagrams/day-23/cardinality-experiment.png)
...
agentgateway 的官方 Dashboard 本來就有 Requests、LLM token／cost、MCP 與 latency 等常用面板，production 應先採用官方版本，再依組織需求擴充。這次沒有複製那些既有 panels，只建立一張 Cardinality 實驗 view：三組互斥的 label 設計必須並排比較，還要把 Prometheus series 與 Loki／Tempo 的單筆查詢放在同一個驗收畫面，這不是 stock Dashboard 能直接回答的問題。

![Day 23 Grafana 實拍。三組 agentgateway 原生 metrics 分別出現 3、30、90 條 series，下方同時顯示 Gateway OTLP access logs。](https://raw.githubusercontent.com/MikeHsu0618/2026-ithelp-agent-governance-public/day-28/assets/screenshots/day-23/agentgateway-cardinality-dashboard.png)
...
完整指令、三份 Gateway YAML、Grafana Dashboard 和 machine-readable evidence 都放在 [Lab 04 README](https://github.com/MikeHsu0618/2026-ithelp-agent-governance-public/blob/day-28/labs/04-telemetry-pipeline/README.md)。截圖只幫忙看出 3／30／90 的差距，PromQL、TraceQL、LogQL 與原始 JSON 都保留成可以複製的文字。
